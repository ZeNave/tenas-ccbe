...
def main(xargs):
    ...
    if xargs.dataset == 'ph2':
        params=PH2_DLV3_FT_params
    elif xargs.dataset == 'derm7':
    ...
    train_data, valid_data, train_loader, valid_loader = data_setup.create_dataloaders(params=Derm7pt_Manually_params,
                                                            train_transform=train_transform,
                                                            val_transform=val_transform)
    
    #print(f"train and valid dataloader: {train_dataloader}  {valid_dataloader}")
    #print(next(iter(train_dataloader)))
    xshape = (1, 3, 32, 32)

    #train_data, valid_data, xshape, class_num = get_datasets(xargs.dataset, xargs.data_path, -1)

    #train_data1, valid_data1, xshape, class_num = get_datasets(xargs.dataset, xargs.data_path, -1)   
    class_num = 2
    ...
    ###############
    logger.log('----------------------------------------------')
    logger.log(f"train_data: \n {train_data}")
    logger.log(f"valid_data: \n {valid_data}")
    logger.log('----------------------------------------------')
    logger.log('----------------------------------------------')
    #logger.log(f"train_data: \n {train_data1}")
    #logger.log(f"valid_data: \n {valid_data1}")
    logger.log('----------------------------------------------')
    #xargs.dataset = 'ph2'
    #print(xargs.dataset == 'cifar10')
    #print(f"xargs.dataset:  {xargs.dataset}")
    ...
    if xargs.search_space_name == 'nas-bench-201':
        ...
        model_config_thin = edict({'name': 'DARTS-V1',
                                   'C': 3, 'N': 1, 'depth': 1, 'use_stem': False,
                                   'max_nodes': xargs.max_nodes, 'num_classes': class_num,
                                   'space': search_space,
                                   'affine': True, 'track_running_stats': bool(xargs.track_running_stats),
                                  })
        ...
        model_config = edict({'name': 'DARTS-V1',
                              'C': 3, 'N': 1, 'depth': 2, 'use_stem': True, 'stem_multiplier': 1,
        ...
        print('-----------SEARCH SPACE -------------')
        print(search_space)
        print(bool(xargs.track_running_stats))
        model_config_thin = edict({'name': 'DARTS-V1',
                                   'C': 3, 'N': 1, 'depth': 2, 'use_stem': False, 'stem_multiplier': 1,
                                   'max_nodes': xargs.max_nodes, 'num_classes': class_num,
                                   'space': search_space,
                                   'affine': True, 'track_running_stats': bool(xargs.track_running_stats),
                                   'super_type': xargs.super_type,
    ...
    print()
    print("++++++++++++++++++++++++++++++")
    print(f"len of train and test data {len(train_data)};    {len(test_data)}")
    # TODO Linear_Region_Collector
    print(f"xargs.data_path:    {xargs.data_path}")
    lrc_model = Linear_Region_Collector(input_size=(15, 3, 32, 32), sample_batch=3, dataset=xargs.dataset, data_path=xargs.data_path, seed=xargs.rand_seed, train_data=train_data, test_data=test_data, train_loader=train_loader, test_loader=test_loader)
    ...
    torch.save(network, 'model_results/model.pth')
    torch.save(network.state_dict(), 'model_state_dict_results/model.pth')
    ...
    #print(f"network.stem_mutltiplier:   {model_config['stem_mutltiplier']}")
    #print(f"network.num_classes:   {model_config['num_classes']}")
    #print(f"network.search_space:   {model_config['search_space']}")
    #print(f"network.affine:   {model_config['affine']}")
    print(f"network.track_running_stats:   {bool(xargs.track_running_stats)}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, choices=['cifar10', 'cifar100', 'ImageNet16-120', 'imagenet-1k', 'derm7'], help='Choose between cifar10/100/ImageNet16-120/imagenet-1k')
